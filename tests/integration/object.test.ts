...
import { createObjectHttpReducer, HttpState } from "../../src/reducer";
...
describe("Object Http Saga Integration", () => {
	const type = "TYPE";
	const path = "https://www.example.com/";
	const method = "POST";
	const actionCreator = createHttpAction<Parameter, Request>(type);
	const key = ({ request }: { request: Request }) => request.bar;

	function filterActions(actions: Action[], id: string): Action[] {
